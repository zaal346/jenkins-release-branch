      ...
      steps {
        container('git') {
          withCredentials([usernamePassword(credentialsId: 'git-credentials-acm', passwordVariable: 'GIT_PASSWORD', usernameVariable: 'GIT_USERNAME')]) {
            ...
            sh "git clone http://${GIT_USERNAME}:${GIT_PASSWORD}@localhost:3000/${env.GITHUB_ORGANIZATION}/${env.SERVICE}"
            sh "version=`cat ${env.SERVICE}/version`"
            sh "cd ${env.SERVICE}/ && git checkout -b release/`cat version`"
            sh "cd ${env.SERVICE}/ && git push --set-upstream http://${GIT_USERNAME}:${GIT_PASSWORD}@localhost:3000/${env.GITHUB_ORGANIZATION}/${env.SERVICE} release/`cat version`"
            ...
            sh "cd ${env.SERVICE}/ && git push http://${GIT_USERNAME}:${GIT_PASSWORD}@localhost:3000/${env.GITHUB_ORGANIZATION}/${env.SERVICE}"
          } 
        }
      }
    }
